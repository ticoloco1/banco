...
export function Header() {
  ...
  const navItems = [
    { href: '/explore/feed', label: T('nav_explore_feed') },
          ...
          <nav style={{ display: 'flex', gap: 6, flex: 1, justifyContent: 'center', flexWrap: 'wrap' }}>
            {navItems.map((nav) => (
              <Link
                ...
                href={nav.href}
                {...(nav.href === '/planos' ? { 'data-tb-analytics': 'nav-planos' } : {})}
                style={linkStyle}
                onMouseEnter={(e) => {
          ...
          >
            {navItems.map((nav) => (
              <Link
                key={`m-${nav.href}`}
                href={nav.href}
                {...(nav.href === '/planos' ? { 'data-tb-analytics': 'nav-planos-mobile' } : {})}
                onClick={() => setOpenMobileNav(false)}
                style={{
                  ...linkStyle,
                  display: 'block',
