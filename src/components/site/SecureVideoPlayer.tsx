...
import { isDbPaywallEnabled, parsePreviewEmbedUrl } from '@/lib/utils';
...
export function SecureVideoPlayer({
  ...
  creatorName, siteSlug, accentColor = '#818cf8', previewImageUrl, previewEmbedUrl,
  ...
  const previewEmbed = parsePreviewEmbedUrl(String(previewEmbedUrl || ''));
  const previewSrc = previewImageUrl ? getAssetUrl(previewImageUrl) : '';
  ...
  const requestToken = async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        // For paywalled videos, show payment options (wallet/stripe) without forcing login.
      ...
      if (!tok || typeof tok !== 'string') throw new Error('Missing token');

      setToken(tok);
      // Verify and get ytId
      const verify = await fetch(`/api/video-token?t=${encodeURIComponent(tok)}`);
      const vdata = (await verify.json().catch(() => ({}))) as {
        valid?: boolean;
        sourceType?: 'yt' | 'embed';
        ytId?: string;
        embedUrl?: string;
      ...
      if (vdata.sourceType === 'embed' && vdata.embedUrl) {
        setEmbedUrl(vdata.embedUrl);
        setYtId(null);
      } else if (vdata.ytId) {
        setYtId(vdata.ytId);
        setEmbedUrl(null);
  ...
  const payWithUsdcPolygon = async () => {
    if (!siteSlug) {
      toast.error('Invalid site for payment');
      return;
    }
    ...
    try {
      ...
      if (!usdc || !isAddress(usdc)) throw new Error('USDC contract is not configured');
      ...
      if (!txHash || typeof txHash !== 'string') throw new Error('Transaction was not sent');
      ...
      console.error(e);
      toast.error(e?.message || 'Error while paying with USDC');
    } finally {
      setWalletBusy(false);
    }
  };
  ...
  // ── States ──────────────────────────────────────────────────────────────────
  if (state === 'playing' && (ytId || embedUrl)) {
    return (
      <div className="relative w-full rounded-2xl overflow-hidden bg-black" style={{ aspectRatio: '16/9' }}>
        <iframe
          ...
          src={
            ytId
              ? `https://www.youtube-nocookie.com/embed/${ytId}?autoplay=1&rel=0&modestbranding=1&iv_load_policy=3&disablekb=0`
              : String(embedUrl || '')
          }
      ...
      <div className="w-full rounded-2xl overflow-hidden" style={{ aspectRatio: '16/9', background: '#0a0015' }}>
        <div className="w-full h-full flex flex-col items-center justify-center gap-4 p-6 text-center">
          ...
          <div>
            ...
            <p className="text-white/50 text-sm mt-1">Sign in with Google to watch</p>
          </div>
    ...
    return (
      <div className="w-full rounded-2xl overflow-hidden" style={{ aspectRatio: '16/9', background: '#0d0d0d' }}>
        <div className="w-full h-full flex flex-col items-center justify-center gap-4 p-4 text-center"
          ...
          {previewEmbed ? (
            <div className="w-full max-w-md rounded-xl overflow-hidden border border-white/15" style={{ aspectRatio: '16/9' }}>
              <iframe
                src={`${previewEmbed}${previewEmbed.includes('?') ? '&' : '?'}rel=0&modestbranding=1`}
                className="w-full h-full border-0"
                allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                allowFullScreen
            ...
            <img
              src={previewSrc}
              className="w-full max-w-md rounded-xl border border-white/15 object-cover"
          ...
          <div>
            <p className="text-white font-black text-xl">Exclusive Content</p>
          ...
          <div className="flex flex-col gap-2 w-full max-w-sm px-2">
            {user && (
              <StripeCheckout
                itemId={videoId}
                label={title ? `Video: ${title}` : 'Video unlock'}
                price={Number(paywallPrice) || 0}
                type="video"
                accentColor={accentColor}
                buttonText="Unlock with card (Stripe)"
            ...
            {!walletAddress ? (
              ...
              <>
                ...
                >
                  {walletBusy ? 'Please wait...' : 'Pay with USDC (Polygon)'}
                </button>
              </>
            )}
            {!user && (
              <a href={`/auth?redirect=${encodeURIComponent(window.location.pathname)}`}
                ...
                Or sign in with Google and pay with Stripe
              </a>
            )}
          </div>
  ...
  if (state === 'error') {
    return (
      <div className="w-full rounded-2xl bg-black flex flex-col items-center justify-center gap-3" style={{ aspectRatio: '16/9' }}>
        <p className="text-white/50 text-sm">Error loading video</p>
  ...
  return (
    <div className="relative w-full rounded-2xl overflow-hidden cursor-pointer group" style={{ aspectRatio: '16/9', background: '#000' }}
      ...
      {previewSrc && (
        <img src={previewSrc} className="absolute inset-0 w-full h-full object-cover" alt={title || 'Video cover'} />
      )}
      {paywallOn && (
        <div className="absolute inset-0" style={{ background: previewSrc ? 'rgba(5,5,16,0.45)' : `radial-gradient(ellipse at 50% 50%, ${accentColor}10 0%, #050510 100%)` }} />
      )}
