...
type Props = {
  ...
  /** Vídeo em loop no círculo do perfil (mesmo estado que o separador Perfil). */
  avatarVideoUrl: string;
  setAvatarVideoUrl: (v: string) => void;
  /** Ficheiro imagem ou vídeo (validação/transcodificação no editor). */
  onProfileMediaFile: (file: File) => Promise<void>;
...
export function CreatorStudioPanel({
  ...
  const onStudioMediaFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const f = e.target.files?.[0];
    e.target.value = '';
    if (!f) return;
    const okImg = f.type.startsWith('image/');
    const okVid = f.type.startsWith('video/') || /\.(mp4|webm|mov)$/i.test(f.name);
    if (!okImg && !okVid) {
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              <input
                ...
                className="hidden"
                onChange={(e) => void onStudioMediaFile(e)}
              />
              <button
                type="button"
