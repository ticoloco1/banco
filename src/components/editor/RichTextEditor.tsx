  ...
  const uploadImageFiles = async (files: FileList | File[]) => {
    ...
    try {
      ...
      for (const file of take) {
        let url: string;
        try {
          // uploads sequenciais para não saturar o browser
          url = await uploadPlatformFileClient(file, 'pages');
        } catch (err) {
          toast.error(err instanceof Error ? err.message : T('rte_image_upload_fail'));
          break;
        }
        if (!/^https:\/\//i.test(url)) {
