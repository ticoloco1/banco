...
export interface MiniSite {
  ...
  /** Vídeo curto (loop) no lugar da foto estática do perfil; opcional. */
  avatar_video_url?: string | null;
  /** Borda neon animada no vídeo de perfil (mini-site público). */
  profile_video_neon_enabled?: boolean | null;
  /** Cor do neon; null = usar accent_color. */
  profile_video_neon_color?: string | null;
  /** Add-on pago: gravação longa (~2 min) no Identity Lab até esta data (UTC). */
  profile_loop_video_extended_until?: string | null;
  ...
  page_modules?: string;
  /** Overlays decorativos do topo (flags/art), com posição no template. */
  template_overlays?: unknown;
  ...
  /** Chaves `lead:uuid` / `nft:uuid` — ordem do showcase Pro no mini-site. */
  featured_partner_assets_order?: string[] | null;
  ...
  /** Diretório global: feed visível até esta data (UTC) após compra de passe. */
  feed_paid_until?: string | null;
  /** Add-on freemium: picker de cor personalizada (Pro tem sempre). */
  freemium_custom_accent_unlocked?: boolean | null;
  ...
  /** Campanha NFT / leads fixada no mini-site (JSON). */
  pinned_campaign?: unknown;
}
...
export function useMySite(options?: UseMySiteOptions) {
  ...
  const save = async (values: Partial<MiniSite>): Promise<MiniSite | null> => {
    ...
    try {
      ...
      const resolveInsertWallet = async (): Promise<string | null> => {
        const w = String(cleanValues.wallet_address || '').trim().toLowerCase();
        if (/^0x[a-f0-9]{40}$/i.test(w)) return w;
        const { data: silent } = await (supabase as any)
          .from('user_silent_wallets')
          .select('address')
          .eq('user_id', user.id)
          .maybeSingle();
        const sw = String((silent as any)?.address || '').trim().toLowerCase();
        return /^0x[a-f0-9]{40}$/i.test(sw) ? sw : null;
      };

      if (preferNew) {
        const resolvedWallet = await resolveInsertWallet();
        const uniqueSlug = `${slug}-${Date.now().toString(36)}`.slice(0, 80);
        const trySlug = async (s: string) => {
          const { data: inserted, error } = await supabase
            .from('mini_sites')
            .insert({ ...cleanValues, wallet_address: resolvedWallet, user_id: user.id, slug: s, updated_at: now })
            .select('*')
            .single();
      ...
      if (existingRow?.id) {
        ...
        const { error } = await supabase
          .from('mini_sites')
          .insert({ ...cleanValues, wallet_address: resolvedWallet, user_id: user.id, slug, updated_at: now })
          .select('*')
          .single();
        if (error) {
