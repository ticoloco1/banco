...
import { CampaignBrandMarquee } from '@/components/site/CampaignBrandMarquee';
import { FeedSponsoredCards } from '@/components/site/FeedSponsoredCards';
...
import { PinnedCampaignSection } from '@/components/site/PinnedCampaignSection';
import { FeaturedAssets } from '@/components/site/FeaturedAssets';
...
import { ProfileLoopVideoPublic } from '@/components/site/ProfileLoopVideoPublic';
...
import { getAssetUrl, getAssetUrls } from '@/lib/getAssetUrl';
import { FeedPostImpression } from '@/components/site/FeedPostImpression';
import { TrafficSessionTracker } from '@/components/analytics/TrafficSessionTracker';
...
const THEMES: Record<string, any> = {
  ...
  rabisco_pop: { bg:'radial-gradient(ellipse 45% 35% at 15% 20%, rgba(251,113,133,0.55) 0%, transparent 60%),radial-gradient(ellipse 42% 30% at 85% 30%, rgba(251,146,60,0.45) 0%, transparent 62%),radial-gradient(ellipse 50% 35% at 60% 85%, rgba(34,211,238,0.4) 0%, transparent 62%),linear-gradient(155deg,#f43f5e 0%,#fb923c 52%,#22d3ee 100%)', text:'#fff7ed', text2:'rgba(255,247,237,0.9)', accent:'#fde047', btn:'rgba(255,255,255,0.22)', btnHover:'rgba(255,255,255,0.34)', border:'rgba(255,255,255,0.32)', radius:18, font:'"Space Grotesk",sans-serif', aurora:true, auroraSoft:true },
  fruta_tropical:{ bg:'radial-gradient(ellipse 48% 34% at 20% 20%, rgba(245,158,11,0.55) 0%, transparent 58%),radial-gradient(ellipse 52% 36% at 80% 22%, rgba(236,72,153,0.45) 0%, transparent 62%),radial-gradient(ellipse 56% 38% at 35% 85%, rgba(132,204,22,0.45) 0%, transparent 60%),linear-gradient(160deg,#fbbf24 0%,#84cc16 45%,#db2777 100%)', text:'#1a2e05', text2:'rgba(26,46,5,0.84)', accent:'#1d4ed8', btn:'rgba(0,0,0,0.14)', btnHover:'rgba(0,0,0,0.22)', border:'rgba(0,0,0,0.22)', radius:16, font:'"DM Sans",sans-serif' },
  mancha_arte: { bg:'radial-gradient(circle at 18% 25%, rgba(249,115,22,0.5) 0%, transparent 35%),radial-gradient(circle at 80% 30%, rgba(244,63,94,0.45) 0%, transparent 38%),radial-gradient(circle at 55% 80%, rgba(99,102,241,0.42) 0%, transparent 40%),repeating-linear-gradient(35deg, rgba(255,255,255,0.12) 0 2px, rgba(255,255,255,0) 2px 12px),linear-gradient(150deg,#fb923c 0%,#f43f5e 42%,#6366f1 100%)', text:'#431407', text2:'rgba(67,20,7,0.82)', accent:'#fef3c7', btn:'rgba(255,255,255,0.18)', btnHover:'rgba(255,255,255,0.3)', border:'rgba(67,20,7,0.24)', radius:14, font:'"Merriweather",serif' },
...
function getPostMedia(post: any): string[] {
  let raw: string[] = [];
  if (Array.isArray(post?.media_urls)) raw = post.media_urls;
...
export default function SitePageClient({
  ...
  const T = useT();
  const feedPaidActive = (() => {
    const raw = (site as any)?.feed_paid_until;
  ...
  const photoSizeMap: Record<string, number> = { xs: 56, sm: 72, md: 96, lg: 128, xl: 192, xxl: 240 };
  ...
  const pageModuleColumnsMap: Record<string, Record<string, 1|2|3>> = (() => {
    ...
    return { home: { pages: 1, links: 1, videos: 1, cv: 1, feed: 1, ads: 1, mystic: 1, slug_market: 1, classified: 1, booking: 1, pinned_campaign: 1 } };
  })();
  ...
  const templateOverlays: PublicTemplateOverlay[] = (() => {
    const raw = (site as any)?.template_overlays;
    const arr = Array.isArray(raw) ? raw : [];
    return arr
      .map((x: any): PublicTemplateOverlay | null => {
        const glyph = String(x?.glyph || '').trim();
        if (!glyph) return null;
        const color = typeof x?.color === 'string' ? x.color : null;
        return {
          id: String(x?.id || ''),
          kind: x?.kind === 'art' ? 'art' : 'flag',
          glyph,
          x: Math.max(0, Math.min(100, Number(x?.x ?? 50))),
          y: Math.max(0, Math.min(100, Number(x?.y ?? 36))),
          size: Math.max(16, Math.min(120, Number(x?.size ?? 34))),
          rotate: Math.max(-180, Math.min(180, Number(x?.rotate ?? 0))),
          opacity: Math.max(0.2, Math.min(1, Number(x?.opacity ?? 1))),
          color,
        };
      })
      .filter(Boolean) as PublicTemplateOverlay[];
  })();
  ...
  const activeModuleCols = pageModuleColumnsMap[activePage] || { pages: 1, links: 1, videos: 1, cv: 1, feed: 1, ads: 1, mystic: 1, slug_market: 1, classified: 1, booking: 1, pinned_campaign: 1 };
  const pageTabsNav =
    sitePages.length > 1 || site?.published ? (
  ...
  if (!site.published && !isOwner) {
    return (
      <div style={{ minHeight: '100vh', background: '#0d1117', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, fontFamily: 'system-ui', padding: 24 }}>
        ...
        <Link
          href="/planos"
          data-tb-analytics="minisite-offline-planos"
          style={{ padding: '12px 24px', borderRadius: 14, background: '#818cf8', color: '#fff', fontWeight: 700, textDecoration: 'none' }}
        >
  ...
  const floatingLivelyActive = showLivelyAvatar;
  ...
  /** CSS-only frame: toggle in editor, or legacy combo when IA is on and corner agent (not speaking profile). */
  const holographicAvatar =
    livelyCentralMagic || (floatingLivelyActive && !livelyProfileAsAvatar);
  ...
  const centerProfilePhotoUrl =
    typeof site?.avatar_url === 'string' && site.avatar_url.trim()
      ? getAssetUrl(site.avatar_url.trim())
      : null;
  const profileLoopVideoRaw =
    typeof site?.avatar_video_url === 'string' && site.avatar_video_url.trim()
      ? site.avatar_video_url.trim()
      : '';
  const profileVideoNeonOn = (site as { profile_video_neon_enabled?: boolean | null })?.profile_video_neon_enabled !== false;
  const profileVideoNeonColorRaw =
    typeof (site as { profile_video_neon_color?: string | null })?.profile_video_neon_color === 'string'
      ? (site as { profile_video_neon_color: string }).profile_video_neon_color.trim()
      : '';
  const profileVideoNeonHex = (() => {
    const hex = profileVideoNeonColorRaw.replace('#', '').toLowerCase();
  ...
  return (
    <div style={{minHeight:'100vh',background:pageBg,fontFamily:t.font,position:'relative',overflowX:'hidden'}}>
      ...
      <CampaignBrandMarquee />
      {/* Slug ticker */}
      <SlugTicker siteUserId={site.user_id} customItems={customTickerItems} enabled={tickerEnabled} />

      <TrafficSessionTracker
        mode="minisite"
        siteId={site.id}
        enabled={Boolean(site.published && !isOwner && !authLoading)}
      />
        ...
        >
          <div
            style={{
              width: '100%',
              height: '100%',
              backgroundImage: `url(${getAssetUrl(site.banner_url)})`,
      ...
      >
        ...
        >
          <CentralProfileMagicAvatar
            ...
            accent={accent}
          >
            <AvatarTiltShell enabled={livelyCentralMagic || floatingLivelyActive}>
              <div
                style={{
                  display: 'inline-block',
                  padding: 3,
                  ...
                  background: holographicAvatar
                    ? `linear-gradient(135deg,${accent},${accent}55,rgba(255,255,255,0.1))`
                    : `linear-gradient(135deg,${accent},${accent}60,rgba(255,255,255,0.12))`,
                  marginBottom: 12,
              ...
              >
                {showProfileSpeakingAvatar ? (
                  ...
                  <ProfileLoopVideoPublic
                    videoPath={profileLoopVideoRaw}
                    posterPath={centerProfilePhotoUrl}
                    size={avatarSize}
                    borderRadius={
                      site.photo_shape === 'round'
                        ? '50%'
                    ...
                    themeBorder={`2px solid ${t.border}`}
                    neonEnabled={profileVideoNeonOn}
                    neonColorHex={profileVideoNeonHex}
                    labelMute={T('site_profile_video_mute_a11y')}
                    labelUnmute={T('site_profile_video_unmute_a11y')}
                  />
                ) : centerProfilePhotoUrl ? (
                  <img
                    className="tb-profile-photo-alive"
                    ...
                    style={{
                      ...
                      display: 'block',
                      border: `2px solid ${t.border}`,
                      animation: 'tbProfileAlive 5.5s ease-in-out infinite',
      ...
      {templateOverlays.map((ov) => (
        <span
          key={ov.id || `${ov.glyph}_${ov.x}_${ov.y}`}
          aria-hidden
          style={{
            position: 'absolute',
            left: `${ov.x}%`,
            ...
            lineHeight: 1,
            opacity: ov.opacity,
            color: ov.kind === 'art' ? (ov.color || accent) : undefined,
            userSelect: 'none',
            pointerEvents: 'none',
            zIndex: 4,
            filter: ov.kind === 'art' ? `drop-shadow(0 0 8px ${(ov.color || accent)}66)` : undefined,
          }}
        >
          {ov.glyph}
        </span>
      ...
      <div style={{ maxWidth: pageMaxWidth, margin: '0 auto', padding: '28px 20px 80px', position: 'relative', zIndex: 3 }}>
        ...
        {(() => {
              ...
              {(site.cv_locked && !cvUnlocked && !cvFree) ? (
                <div style={{padding:20,borderRadius:r,border:`1.5px solid ${accent}35`,background:`${accent}0a`}}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,flexWrap:'wrap'}}>
                    ...
                    <button
                      type="button"
                      data-tb-analytics="paywall-cv-unlock"
                      onClick={handleCvUnlock}
                      style={{padding:'11px 22px',borderRadius:999,background:`linear-gradient(135deg,${accent},${accent}cc)`,color:'#fff',fontWeight:800,fontSize:13,border:'none',cursor:'pointer',whiteSpace:'nowrap'}}
                    >
                ...
                {videos.map((v:any) => (
                  <div key={v.id} style={{borderRadius:r,overflow:'hidden',border:`1.5px solid ${t.border}`}}>
                    <SecureVideoPlayer
                      ...
                      accentColor={accent}
                      siteSlug={slug}
                      previewImageUrl={v.preview_image_url || null}
          ...
          if (mod === 'feed' && (site as any).show_feed !== false) {
            if (!feedPaidActive && !canManageFeed) return null;
            if (!feedPaidActive && canManageFeed && site.id) {
              return (
                <div key="feed" style={{ marginBottom: 32 }}>
                  ...
                  >
                    <p style={{ margin: '0 0 8px', fontWeight: 800, fontSize: 15, color: t.text }}>{T('feed_paywall_title')}</p>
                    <p style={{ margin: '0 0 16px', fontSize: 12, color: t.text2, lineHeight: 1.6 }}>{T('feed_paywall_body')}</p>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
                      <button
                        type="button"
                        onClick={() => {
                          addToCart({
                            id: `feed_pass_7d_${site.id}`,
                            label: T('feed_cart_7d'),
                            price: PLATFORM_USD.feedDirectory7d,
                      ...
                      >
                        {T('feed_buy_7d').replace('{p}', String(PLATFORM_USD.feedDirectory7d))}
                      </button>
                      <button
                        type="button"
                        onClick={() => {
                          addToCart({
                            id: `feed_pass_365d_${site.id}`,
                            label: T('feed_cart_365d'),
                            price: PLATFORM_USD.feedDirectory365d,
                            type: 'feed_subscription',
                          });
                          openCart();
                        }}
                      ...
                      >
                        {T('feed_buy_365d').replace('{p}', String(PLATFORM_USD.feedDirectory365d))}
                      </button>
                    </div>
                    <p style={{ margin: '14px 0 0', fontSize: 10, color: t.text2 }}>{T('feed_paywall_explore_hint')}</p>
                  </div>
                </div>
              );
            }
            ...
            <div key="feed" style={{marginBottom:32}}>
              {/* Owner composer */}
              {feedPaidActive && canManageFeed && site.id && activeModules.includes('feed') && (
          ...
          if (mod === 'pinned_campaign') {
            const cfg = (site as any).pinned_campaign as Record<string, unknown> | null | undefined;
            if (!cfg || cfg.enabled !== true) return null;
            return (
              <div key="pinned_campaign" style={{ marginBottom: 32 }}>
                <PinnedCampaignSection
                  siteSlug={slug}
                  config={cfg as any}
                  accentColor={accent}
                  textColor={textMain}
              ...
              {videos.map((v:any) => (
                <div key={`wall_${v.id}`} style={{borderRadius:r,overflow:'hidden',border:`1.5px solid ${t.border}`}}>
                  <SecureVideoPlayer
                    ...
                    accentColor={accent}
                    siteSlug={slug}
                    previewImageUrl={v.preview_image_url ? getAssetUrl(v.preview_image_url) : null}
                    previewEmbedUrl={v.preview_embed_url}
                  />
                </div>
              ))}
        ...
        {lightboxImage && (
          ...
          >
            <img src={getAssetUrl(lightboxImage)} style={{maxWidth:'95vw',maxHeight:'92vh',objectFit:'contain',borderRadius:10}} />
          </button>
        )}
      ...
      @keyframes tbProfileAlive{0%,100%{transform:translateY(0) scale(1)}50%{transform:translateY(-4px) scale(1.025)}}
      @media (prefers-reduced-motion:reduce){.tb-profile-photo-alive{animation:none!important}}
