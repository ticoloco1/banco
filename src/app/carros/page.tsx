  ...
  return (
    <div className="relative aspect-[16/10] overflow-hidden group bg-black">
      <div className="absolute inset-0 transition-all duration-400" style={{ transform: `translateX(-${idx * 100}%)`, display: 'flex', width: `${images.length * 100}%` }}>
        {images.map((img, i) => (
          <img key={i} src={getAssetUrl(img)} alt={`${title} ${i + 1}`}
          ...
          <div>
            ...
            {item.mini_sites && (
              <p className="text-xs text-zinc-500 flex items-center gap-1 mt-0.5">
                {item.mini_sites.avatar_url && <img src={getAssetUrl(item.mini_sites.avatar_url)} className="w-4 h-4 rounded-full" />}
        ...
        <div className="bg-white dark:bg-zinc-900 w-full max-w-3xl max-h-[95vh] overflow-y-auto rounded-t-3xl sm:rounded-2xl shadow-2xl" onClick={e => e.stopPropagation()}>
          ...
          <div className="relative rounded-t-2xl overflow-hidden">
            {images.length === 0 ? (
              ...
              <img src={getAssetUrl(images[0])} className="w-full aspect-[16/9] object-cover cursor-zoom-in" onClick={() => setLightboxIdx(0)} />
            ) : (
              <div className="grid grid-cols-4 grid-rows-2 gap-1 h-72">
                <div className="col-span-2 row-span-2 overflow-hidden cursor-zoom-in" onClick={() => setLightboxIdx(0)}>
                  <img src={getAssetUrl(images[0])} className="w-full h-full object-cover hover:brightness-90 transition-all" />
                </div>
                {images.slice(1, 5).map((img, i) => (
                  <div key={i} className="overflow-hidden relative cursor-zoom-in" onClick={() => setLightboxIdx(i + 1)}>
                    <img src={getAssetUrl(img)} className="w-full h-full object-cover hover:brightness-90 transition-all" />
                    {i === 3 && images.length > 5 && (
          ...
          <div className="p-6">
            ...
            {typeof item.extra?.lat === 'number' && typeof item.extra?.lng === 'number' && (
              <div className="mb-6">
                ...
                {item.extra.map_label && (
                  <p className="text-xs text-zinc-500 mt-2">{item.extra.map_label}</p>
                )}
              </div>
            )}
            ...
            {item.mini_sites && (
              <div className="border border-zinc-200 dark:border-zinc-700 rounded-2xl p-4 flex items-center gap-4">
                {item.mini_sites.avatar_url ? <img src={getAssetUrl(item.mini_sites.avatar_url)} className="w-12 h-12 rounded-full object-cover" /> : <div className="w-12 h-12 rounded-full bg-blue-100 dark:bg-blue-950 flex items-center justify-center"><Car className="w-5 h-5 text-blue-500" /></div>}
