...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    supabase.from('user_roles' as any).select('role').eq('user_id', user.id).eq('role', 'admin').maybeSingle()
      .then(({ data }) => {
        if (data) { setIsAdmin(true); setChecking(false); loadAll(); }
        else { router.push('/'); }
      });
  }, [user, loading]);
  ...
  const uploadFavicon = async (file: File) => {
    try {
      ...
      const url = await uploadPlatformFileClient(file, 'favicon', { admin: true });
      setFaviconUrl(url);
      toast.success(T('gov_toast_favicon'));
    } catch (e: unknown) {
      toast.error(e instanceof Error ? e.message : T('toast_upload_failed'));
    }
  };
      ...
      <div className="border-b border-[var(--border)] bg-[var(--bg)] sticky top-0 z-50">
        <div className="max-w-screen-2xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {faviconUrl && <img src={getAssetUrl(faviconUrl)} alt="" className="w-7 h-7 rounded" />}
                ...
                <div className="space-y-2">
                  {sites.slice(0, 8).map(site => (
                    <div key={site.id} className="flex items-center justify-between py-2 border-b border-[var(--border)] last:border-0">
                      <div className="flex items-center gap-3">
                        {site.avatar_url
                          ? <img src={getAssetUrl(site.avatar_url)} alt="" className="w-8 h-8 rounded-full object-cover" />
                ...
                {sites
                  ...
                  <div key={site.id} className="card p-4">
                    <div className="flex items-center gap-3 mb-3">
                      {site.avatar_url
                        ? <img src={getAssetUrl(site.avatar_url)} alt="" className="w-10 h-10 rounded-full object-cover" />
              ...
              <div className="card p-6 space-y-5">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    {faviconUrl && <img src={getAssetUrl(faviconUrl)} alt="favicon" className="w-10 h-10 rounded-lg border border-[var(--border)]" />}
