...
import {
  Edit3, ExternalLink, Key, TrendingUp, BarChart3,
  Trash2, Plus, Loader2, Megaphone, Sparkles,
  Home, Car, Video, FileText, AlertTriangle, ShieldCheck,
} from 'lucide-react';
...
/** Após Stripe: ?payment=success&session_id=cs_… — ativa anúncios (imóveis/carros) se o webhook falhou. */
function CheckoutReturnSync() {
  ...
  useEffect(() => {
    ...
    if (pay !== 'success' || !sid || !sid.startsWith('cs_')) return;
    ran.current = true;

    void (async () => {
      toast.message(T('toast_payment_confirming'));
      try {
        ...
        if (res.ok) {
          toast.success(T('toast_checkout_fulfilled'));
        } else {
          toast.error(j.error || T('toast_checkout_error'));
        }
      } catch {
...
type AdProposalTargetRow = {
  ...
  ad_proposals: {
    ...
    status: string;
    magic_script: string | null;
    body: string | null;
    magic_product_label: string | null;
    created_at: string;
  } | null;
};
...
export default function DashboardPage() {
  ...
  const [tab, setTab] = useState<'overview' | 'slugs' | 'feed' | 'listings' | 'ad_proposals'>('overview');
  const [auctionWinsPending, setAuctionWinsPending] = useState<any[]>([]);
  const { add: addToCart, open: openCart } = useCart();

  const feedPaidOk = Boolean(
    site?.feed_paid_until && new Date(site.feed_paid_until) > new Date(),
  );
  ...
  const [adTargets, setAdTargets] = useState<AdProposalTargetRow[]>([]);
  const [loadingAdTargets, setLoadingAdTargets] = useState(false);
  const [acceptingTargetId, setAcceptingTargetId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (em === DASH_OWNER_EMAIL.toLowerCase()) {
        if (!cancelled) setPlatformAdmin(true);
        return;
      }
      const { data } = await (supabase as any)
        ...
        .eq('role', 'admin')
        .maybeSingle();
      if (!cancelled) setPlatformAdmin(!!data);
    })();
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    if (!site?.id) return;
    let cancelled = false;
    setLoadingAdTargets(true);
    void (async () => {
      const { data, error } = await (supabase as any)
        .from('ad_proposal_targets')
        .select(`
          id,
          bid_amount_usdc,
          owner_status,
      ...
      if (error) {
        setAdTargets([]);
        setLoadingAdTargets(false);
        return;
      }

      const rows = ((data || []) as AdProposalTargetRow[])
        .sort((a, b) => {
          const da = new Date(a.ad_proposals?.created_at || 0).getTime();
          const db = new Date(b.ad_proposals?.created_at || 0).getTime();
          return db - da;
        });
      setAdTargets(rows);
      setLoadingAdTargets(false);
  ...
  const TABS = [
    ...
    {
      id: 'ad_proposals',
      label: `✨ ${T('dash_ad_proposals_tab')} (${adTargets.filter((r) => r.owner_status === 'pending').length})`,
      icon: Megaphone,
    },
  ];
        ...
        {tab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="card p-5 flex items-center gap-4">
              {site?.avatar_url
                ? <img src={getAssetUrl(site.avatar_url)} className="w-14 h-14 rounded-2xl object-cover flex-shrink-0" />
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Link href="/dashboard/campanhas" className="btn-secondary text-sm py-2 px-3">
                  NFT Leads
                </Link>
                {site?.slug && (
                  <Link
                    href={`/stats/${encodeURIComponent(site.slug)}`}
                    className="btn-secondary text-sm py-2 px-3 inline-flex items-center gap-1.5"
                  >
                    <BarChart3 className="w-4 h-4" />
                    {T('dashboard_traffic_stats')}
                  </Link>
                )}
                {site?.slug && (
                  <a
            ...
            <div className="card p-5 space-y-3 border border-brand/20 bg-brand/[0.04]">
              <p className="text-sm font-black text-[var(--text)]">{T('dash_freemium_addons_title')}</p>
              <p className="text-xs text-[var(--text2)] leading-relaxed">{T('dash_freemium_addons_sub')}</p>
              <ul className="text-xs text-[var(--text)] space-y-2 border-t border-[var(--border)]/60 pt-3">
                <li className="flex justify-between gap-2">
                  <span>{T('dash_freemium_row_feed_7')}</span>
                  <span className="font-mono font-bold shrink-0">${PLATFORM_USD.feedDirectory7d}</span>
                </li>
                <li className="flex justify-between gap-2">
                  <span>{T('dash_freemium_row_feed_365')}</span>
                  <span className="font-mono font-bold shrink-0">${PLATFORM_USD.feedDirectory365d}</span>
                </li>
                <li className="flex justify-between gap-2">
                  <span>{T('dash_freemium_row_directory')}</span>
                  <span className="font-mono font-bold shrink-0">${PLATFORM_USD.slugHighlight30d}</span>
                </li>
              </ul>
              <div className="flex flex-wrap gap-2 pt-1">
                {site?.id ? (
                  <>
                    ...
                    >
                      {T('dash_freemium_cta_feed_365')}
                    </button>
                    <Link href="/slugs" className="btn-secondary text-xs inline-flex items-center justify-center">
                      {T('dash_freemium_cta_directory')}
                    </Link>
                  </>
                ) : (
                  <Link href="/editor" className="btn-primary text-xs inline-flex items-center justify-center">
                    {T('dash_freemium_cta_create_site')}
                  </Link>
                )}
              </div>
            </div>
  ...
  {T('dash_view_site')}
            ...
            {site?.id && !feedPaidOk && (
              <div className="rounded-2xl border border-brand/30 bg-brand/5 p-5 space-y-3">
                <p className="font-black text-[var(--text)]">{T('feed_paywall_title')}</p>
                <p className="text-xs text-[var(--text2)] leading-relaxed">{T('feed_paywall_body')}</p>
                <div className="flex flex-wrap gap-2">
                  <button
                    type="button"
                    onClick={() => {
                ...
                <Link href="/explore/feed" className="text-xs font-bold text-brand hover:underline inline-block">
                  {T('feed_paywall_explore_hint')} →
                </Link>
              </div>
            )}
              ...
              <div className="space-y-3">
                {listings.map((l: any) => (
                  <div key={l.id} className="card p-4 flex items-center gap-3">
                    {l.images?.[0]
                      ? <img src={getAssetUrl(l.images[0])} className="w-16 h-12 rounded-xl object-cover flex-shrink-0" />
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {platformAdmin && l.status === 'pending' && (
                        <button
                          type="button"
                          title={T('dash_listing_admin_approve_title')}
                          disabled={approvingListingId === l.id}
                          onClick={async () => {
                            setApprovingListingId(l.id);
                            try {
                              const res = await fetch('/api/admin/classified-approve', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                credentials: 'include',
                              ...
                              toast.success(T('toast_listing_admin_approved'));
                            } finally {
                              setApprovingListingId(null);
                            }
                          }}
                          className="text-emerald-400 hover:opacity-80 disabled:opacity-40"
                        >
                          {approvingListingId === l.id ? (
        ...
        {tab === 'ad_proposals' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-black text-[var(--text)]">{T('dash_ad_proposals_title')}</p>
                <p className="text-xs text-[var(--text2)]">{T('dash_ad_proposals_sub')}</p>
              </div>
            </div>
            ...
            {loadingAdTargets ? (
              ...
              <div className="card p-10 text-center">
                <Megaphone className="w-12 h-12 text-[var(--text2)] mx-auto mb-3 opacity-20" />
                <p className="font-bold text-[var(--text)] mb-1">{T('dash_ad_proposals_empty')}</p>
                <p className="text-sm text-[var(--text2)]">{T('dash_ad_proposals_empty_sub')}</p>
              </div>
            ) : (
              <div className="space-y-3">
                {adTargets.map((row) => {
                  const p = row.ad_proposals;
                  const isPending = row.owner_status === 'pending';
                  const suggestedScript = (row.owner_ai_script || p?.magic_script || p?.body || '').trim();
                  const slotRaw = typeof row.message_to_owner === 'string' ? row.message_to_owner : '';
                  const slotLegacy = slotRaw.startsWith('SLOTS:') ? slotRaw.replace('SLOTS:', '').split(',').map((s) => s.trim()).filter(Boolean) : [];
                  const slotList = Array.isArray(row.ad_slots) && row.ad_slots.length ? row.ad_slots : slotLegacy;
                  return (
                    <div
                      key={row.id}
                      className="rounded-2xl border p-4"
                      style={{
                        borderColor: isPending ? 'rgba(245, 158, 11, 0.5)' : 'hsl(var(--border))',
                    ...
                    >
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <p className="text-sm font-black text-[var(--text)]">{p?.title || T('dash_ad_proposals_offer')}</p>
                          <p className="text-xs text-[var(--text2)] mt-1">
                            {T('dash_ad_proposals_term').replace('{days}', String(p?.duration_days || 7))}
                            {' · '}
                            {T('dash_ad_proposals_hold').replace('{status}', String(p?.payment_status || 'unpaid'))}
                          </p>
                        </div>
                        <span className="px-3 py-1 rounded-full text-xs font-black text-amber-100 bg-amber-500/80">
                          ${Number(row.bid_amount_usdc || 0).toFixed(2)}
                        </span>
                      </div>

                      {suggestedScript && (
                        <div className="mt-3 p-3 rounded-xl border border-amber-300/30 bg-black/20">
                          <p className="text-[11px] uppercase tracking-wide font-bold text-amber-200 mb-1 flex items-center gap-1">
                            <Sparkles className="w-3.5 h-3.5" />
                            {T('dash_ad_proposals_script')}
                          </p>
                          <p className="text-sm text-[var(--text)] whitespace-pre-wrap">{suggestedScript}</p>
                        </div>
                      )}
                      {slotList.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-1.5">
                          {slotList.map((slot) => (
                            <span key={slot} className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-500/15 text-amber-200 border border-amber-500/30">
                              {slot === 'feed_pinned'
                                ? T('mkt_slot_feed')
                                : slot === 'avatar_ai'
                                  ? T('mkt_slot_avatar')
                      ...
                      <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                        <span className="text-xs text-[var(--text2)]">
                          {row.owner_status === 'accepted'
                            ? T('dash_ad_proposals_status_accepted')
                            : row.owner_status === 'rejected'
                              ? T('dash_ad_proposals_status_rejected')
                              : T('dash_ad_proposals_status_pending')}
                        </span>
                        ...
                        {isPending ? (
                          <button
                            type="button"
                            onClick={async () => {
                              setAcceptingTargetId(row.id);
                              try {
                                const resp = await fetch('/api/ad-proposals/accept', {
                                  method: 'POST',
                                  headers: { 'Content-Type': 'application/json' },
                                  body: JSON.stringify({ targetId: row.id }),
                                });
                                const json = await resp.json().catch(() => ({}));
                                if (!resp.ok) throw new Error(json?.error || 'Falha ao aceitar');
                                toast.success(T('dash_ad_proposals_accept_ok'));
                                setAdTargets((prev) =>
                                  prev.map((it) =>
                                    it.id === row.id
                                      ? {
                                          ...
                                          ad_proposals: it.ad_proposals
                                            ? { ...it.ad_proposals, payment_status: json?.holdStatus || it.ad_proposals.payment_status }
                                            : it.ad_proposals,
                                        }
                                      : it,
                                  ),
                                ...
                                toast.error(e?.message || T('dash_ad_proposals_accept_err'));
                              } finally {
                                setAcceptingTargetId(null);
                              }
                            }}
                            disabled={acceptingTargetId === row.id}
                            className="btn-primary text-sm py-2 px-4"
                          >
                            {acceptingTargetId === row.id ? T('admin_loading') : T('dash_ad_proposals_accept')}
                          </button>
                        ) : null}
                      </div>
                    </div>
