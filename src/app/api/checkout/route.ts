...
export async function POST(req: Request) {
  try {
    ...
    const needsSlugWallet = lines.some((l) =>
      l.kind === 'slug' || l.kind === 'slug_market' || l.kind === 'slug_auction_settle',
    );
    let effectivePolygon: string | null = polygonWallet;
    if (needsSlugWallet) {
      if (!effectivePolygon || !isValidEvmAddress(effectivePolygon)) {
        const fromProfile = await getUserPolygonWallet(db, userId);
        effectivePolygon = fromProfile && isValidEvmAddress(fromProfile) ? fromProfile : null;
      }
      if (!effectivePolygon) {
        return NextResponse.json(
          {
            error:
              'Carteira Polygon obrigatória para comprar ou registar slug (NFT). Define no editor/KYC ou no carrinho antes de pagar.',
          },
          { status: 400 },
        );
      }
      polygonWallet = effectivePolygon.toLowerCase();
    }
