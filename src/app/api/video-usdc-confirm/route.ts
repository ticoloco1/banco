...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!intentId || !txHash || !walletAddress) {
      return NextResponse.json({ error: 'intentId, txHash and walletAddress are required' }, { status: 400 });
    }
    if (!isAddress(walletAddress) || !/^0x[a-fA-F0-9]{64}$/.test(txHash)) {
      return NextResponse.json({ error: 'Invalid walletAddress/txHash' }, { status: 400 });
    }
    ...
    if (i.payer_wallet.toLowerCase() !== walletAddress) {
      return NextResponse.json({ error: 'Wallet does not match intent' }, { status: 400 });
    }
    if (i.status === 'paid') {
    ...
    if (!okTransfer) {
      return NextResponse.json({ error: 'USDC transfer was not confirmed with expected amount' }, { status: 400 });
    }
