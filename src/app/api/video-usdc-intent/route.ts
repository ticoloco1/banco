...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!videoId || !siteSlug || !walletAddress) {
      return NextResponse.json({ error: 'videoId, siteSlug and walletAddress are required' }, { status: 400 });
    }
    ...
    if (s.slug.toLowerCase() !== siteSlug) return NextResponse.json({ error: 'Invalid slug' }, { status: 400 });
    if (!s.published) return NextResponse.json({ error: 'Unavailable' }, { status: 403 });
    if (!isDbPaywallEnabled((video as { paywall_enabled?: unknown }).paywall_enabled)) {
      return NextResponse.json({ error: 'Video is not paywalled' }, { status: 400 });
    }

    const amountUsd = Number((video as { paywall_price?: number | null }).paywall_price || 0);
    if (!Number.isFinite(amountUsd) || amountUsd <= 0) {
      return NextResponse.json({ error: 'Invalid video price' }, { status: 400 });
    }
