...
import { Check, Loader2, Zap, Sparkles, Gift } from 'lucide-react';
...
export default function PlanosPage() {
  ...
  const feedNote = T('plans_freemium_feed_note')
    .replace('{d7}', String(PLATFORM_USD.feedDirectory7d))
    .replace('{d365}', String(PLATFORM_USD.feedDirectory365d));
    ...
    <div className="min-h-screen bg-[var(--bg)]">
      ...
      <div className="max-w-5xl mx-auto px-4 py-16">
        <div className="text-center mb-10">
          ...
          <p className="text-sm text-[var(--text2)] mt-4 leading-relaxed max-w-2xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="grid md:grid-cols-2 gap-8 items-stretch">
            {/* Freemium */}
            <div className="card p-8 border border-[var(--border)] flex flex-col relative">
              <div className="absolute -top-3 left-6 bg-[var(--bg2)] border border-[var(--border)] text-[var(--text2)] text-xs font-black px-3 py-1 rounded-full">
                {T('plans_freemium_badge')}
              </div>
              <div className="mb-6 mt-2">
                <span className="text-3xl">
                  <Gift className="w-8 h-8 inline text-emerald-400" />
                </span>
                <h2 className="text-xl font-black mt-2 text-[var(--text)]">{T('plans_freemium_name')}</h2>
                <div className="flex flex-wrap items-end gap-2 mt-3">
                  <span className="text-4xl font-black text-[var(--text)]">{T('plans_freemium_price')}</span>
                  <span className="text-[var(--text2)] mb-1 text-sm">{lang === 'pt' ? '/mês' : '/mo'} · —</span>
                </div>
                <p className="text-xs text-[var(--text2)] mt-2 leading-relaxed">{T('plans_freemium_sub')}</p>
              </div>

              <ul className="space-y-2.5 mb-4 flex-1">
                {freemiumFeatures.map((f: string, i: number) => (
              ...
              <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 mb-4 text-xs text-[var(--text2)] leading-relaxed space-y-2">
                <p>{feedNote}</p>
                <p className="text-[var(--text)] font-semibold">{T('plans_freemium_split_cv')}</p>
                <p>{T('plans_freemium_split_pro_cv')}</p>
              </div>

              <Link href="/editor" className="btn-secondary w-full text-center py-3.5 rounded-xl font-black text-base">
                {T('plans_freemium_cta')}
              </Link>
            </div>

            {/* Pro */}
            <div className="card p-8 relative ring-2 ring-brand shadow-xl shadow-brand/20 flex flex-col">
              ...
              <div className="flex items-center justify-center gap-3 mt-2 mb-6">
                ...
                >
                  <div
                    className={`absolute top-1 w-5 h-5 rounded-full bg-white shadow transition-all ${annual ? 'left-8' : 'left-1'}`}
                  />
                </button>
                <span className={`text-sm font-semibold ${annual ? 'text-[var(--text)]' : 'text-[var(--text2)]'}`}>
                  {T('plans_yearly')}
              ...
              <button
                type="button"
                onClick={handleBuy}
                className="w-full py-3.5 rounded-xl font-black text-base transition-all hover:opacity-90 btn-primary mt-auto"
              >
