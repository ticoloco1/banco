...
// ─── CV Card ──────────────────────────────────────────────────────────────────
function CVCard({ profile, onUnlock, matchScore }: { profile: CVProfile; onUnlock: (p: CVProfile) => void; matchScore?: number | null }) {
  const T = useT();
  ...
  return (
    <div className="card p-5 hover:border-brand/40 transition-all duration-200 hover:-translate-y-0.5 group">
      ...
      <div className="flex items-start gap-3 mb-4">
        {profile.avatar_url
          ? <img src={getAssetUrl(profile.avatar_url)} className="w-12 h-12 rounded-xl object-cover flex-shrink-0" style={{ border: `2px solid ${accent}40` }} />
      ...
      {typeof matchScore === 'number' && (
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold mb-3 ml-2 bg-violet-500/10 text-violet-300 border border-violet-500/25">
          <Star className="w-3 h-3" />
          {T('cv_match_score').replace('{n}', String(Math.round(matchScore)))}
        </div>
      )}
...
// ─── Main Page ────────────────────────────────────────────────────────────────
const PAGE_SIZE = 12;
const MATCH_LEVELS = [50, 60, 70, 80, 90, 100] as const;

const EXAMPLE_CVS: CVProfile[] = [
  {
    id: 'example-cv-1',
    slug: 'demo-ceo-ops',
    site_name: 'Amanda CEO Ops',
    cv_headline: 'CEO / COO · Escala de SaaS B2B',
    cv_location: 'Remote · USA',
    cv_skills: ['Leadership', 'SaaS', 'Growth', 'Finance'],
    cv_hire_price: 500,
    cv_hire_currency: 'USD',
    cv_hire_type: 'hour',
    cv_free: true,
    cv_price: 0,
    cv_locked: false,
    accent_color: '#6366f1',
    cv_experience: [{ company: 'ScaleForge', role: 'CEO', current: true }],
    cv_languages: [{ lang: 'English', level: 'Fluent' }],
    is_example: true,
  },
  {
    id: 'example-cv-2',
    slug: 'demo-cto-ai',
    site_name: 'Rafael CTO AI',
    cv_headline: 'CTO · IA aplicada a produto',
    cv_location: 'Brasil · Remote',
    cv_skills: ['Deep Learning', 'MLOps', 'Architecture', 'Hiring'],
    cv_hire_price: 380,
    cv_hire_currency: 'USD',
    cv_hire_type: 'hour',
    cv_free: true,
    ...
    cv_locked: false,
    accent_color: '#22d3ee',
    cv_experience: [{ company: 'NeuralGrid', role: 'CTO', current: true }],
    cv_languages: [{ lang: 'Português', level: 'Native' }, { lang: 'English', level: 'Fluent' }],
    is_example: true,
  },
  {
    id: 'example-cv-3',
    slug: 'demo-cfo-growth',
    site_name: 'Bianca CFO Growth',
    cv_headline: 'CFO · M&A · Planejamento financeiro',
    cv_location: 'Europe',
    cv_skills: ['Finance', 'FP&A', 'M&A', 'Investor Relations'],
    cv_hire_price: 420,
    cv_hire_currency: 'USD',
    cv_hire_type: 'hour',
    cv_free: true,
    ...
    cv_locked: false,
    accent_color: '#f59e0b',
    cv_experience: [{ company: 'Atlas Capital', role: 'CFO', current: true }],
    cv_languages: [{ lang: 'English', level: 'Fluent' }, { lang: 'Spanish', level: 'Advanced' }],
    is_example: true,
  },
];
...
export default function CVDirectoryPage() {
  ...
  const [includedRemaining, setIncludedRemaining] = useState<number>(20);
  ...
  const tokenize = (v: string) =>
    v.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').split(/[^a-z0-9]+/i).filter((x) => x.length >= 2);

  const localMatchScore = (p: CVProfile, need: string) => {
    const q = tokenize(need);
    ...
    const hay = [
      ...
      p.cv_location,
      ...(p.cv_skills || []),
      ...(p.cv_experience || []).map((e) => `${e.role} ${e.company}`),
    ].join(' ').toLowerCase();
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const [{ data: roleRow }, { data: accessRow }, { data: companyRow }] = await Promise.all([
        ...
        supabase.from('company_profiles' as any).select('company_name').eq('user_id', user.id).maybeSingle(),
      ]);
      if (cancelled) return;
      setIsAdmin(!!roleRow);
      const exp = accessRow?.expires_at as string | undefined;
      setDirectoryPaid(!!(exp && new Date(exp) > new Date()));
      setCompanyName(String((companyRow as any)?.company_name || ''));
      setAccessChecked(true);

      const [{ data: unlockRows, count }] = await Promise.all([
        supabase.from('cv_unlocks' as any).select('site_id').eq('unlocker_id', user.id),
        supabase
          .from('cv_unlocks' as any)
          .select('id', { count: 'exact', head: true })
          .eq('unlocker_id', user.id)
          .eq('source', 'directory_included')
          .gte('created_at', new Date(Date.now() - 30 * 24 * 3600 * 1000).toISOString()),
      ]);
      if (!cancelled) {
        setUnlockedSiteIds(new Set((unlockRows || []).map((r: any) => String(r.site_id))));
        const used = typeof count === 'number' ? count : 0;
        setIncludedRemaining(Math.max(0, 20 - used));
      }
    })();
  ...
  const handleUnlock = async (profile: CVProfile) => {
    if (!user) { toast.error(T('toast_login_unlock_cv')); return; }
    if (unlockedSiteIds.has(profile.id)) {
      toast.success(T('cv_already_unlocked'));
      return;
    }
    if (directoryPaid || isAdmin) {
      try {
        const res = await fetch('/api/cv/unlock-directory', {
          method: 'POST',
        ...
        if (res.ok && json?.included) {
          setUnlockedSiteIds((prev) => new Set([...prev, profile.id]));
          if (typeof json?.remainingIncluded === 'number') setIncludedRemaining(json.remainingIncluded);
          toast.success(T('cv_unlock_included_ok').replace('{n}', String(json.remainingIncluded ?? 0)));
          return;
        }
      } catch {
        // fallback to paid cart
      }
    }
  ...
  const runAiMatch = async () => {
    if (!jobNeed.trim()) {
      toast.error(T('cv_ai_need_required'));
      return;
    }
    setAiMatching(true);
    try {
      const candidates = profiles.slice(0, 60).map((p) => ({
        id: p.id,
        site_name: p.site_name,
        cv_headline: p.cv_headline,
        cv_location: p.cv_location,
        cv_skills: p.cv_skills || [],
        top_experience: p.cv_experience?.[0] ? `${p.cv_experience[0].role} @ ${p.cv_experience[0].company}` : '',
      }));
      const res = await fetch('/api/cv/match-assist', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ jobNeed, candidates }),
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(json?.error || 'Falha IA');
      const map: Record<string, number> = {};
      for (const row of (json?.scores || [])) map[String(row.id)] = Number(row.score) || 0;
      setAiScores(map);
      toast.success(T('cv_ai_done'));
    } catch (e: any) {
      toast.error(e?.message || T('cv_ai_error'));
    } finally {
      setAiMatching(false);
    }
  };
  ...
  const visibleProfiles = (() => {
    const merged = [...EXAMPLE_CVS, ...profiles];
    if (!jobNeed.trim()) return merged;
    return merged.filter((p) => {
      const score = useAiMatch && aiScores[p.id] != null ? aiScores[p.id] : localMatchScore(p, jobNeed);
    ...
    <div className="min-h-screen bg-[var(--bg)]">
      ...
      <div className="border-b border-[var(--border)] bg-[var(--bg2)]">
        <div className="max-w-6xl mx-auto px-4 py-10">
          ...
          {(directoryPaid || isAdmin) && (
            <div className="mb-4 flex flex-wrap gap-2">
              {companyName ? (
                <span className="inline-flex items-center gap-2 rounded-full border border-indigo-500/25 bg-indigo-500/10 px-3 py-1.5 text-xs font-bold text-indigo-300">
                  <Building2 className="w-3.5 h-3.5" />
                  {companyName}
                </span>
              ) : null}
              <span className="inline-flex items-center gap-2 rounded-full border border-emerald-500/25 bg-emerald-500/10 px-3 py-1.5 text-xs font-bold text-emerald-300">
                <CheckCircle className="w-3.5 h-3.5" />
                {T('cv_included_left').replace('{n}', String(includedRemaining))}
              </span>
            </div>
          )}
          ...
          <div className="card p-3 mb-4 border border-violet-500/25 bg-violet-500/5">
            <p className="text-xs font-bold text-violet-300 mb-2">{T('cv_ai_title')}</p>
            <div className="grid md:grid-cols-4 gap-2">
              <input
                value={jobNeed}
                onChange={(e) => setJobNeed(e.target.value)}
                className="input text-sm md:col-span-2"
                placeholder={T('cv_ai_need_ph')}
              />
              <select
                className="input text-sm"
                value={String(matchThreshold)}
                onChange={(e) => setMatchThreshold(Number(e.target.value) || 80)}
              >
                {MATCH_LEVELS.map((m) => (
                  <option key={m} value={m}>{T('cv_ai_match').replace('{n}', String(m))}</option>
                ))}
              </select>
              <button type="button" disabled={aiMatching || !useAiMatch} onClick={() => void runAiMatch()} className="btn-primary text-sm justify-center">
                {aiMatching ? <Loader2 className="w-4 h-4 animate-spin" /> : <Star className="w-4 h-4" />}
                {T('cv_ai_run')}
              </button>
            </div>
            <div className="flex items-center gap-2 mt-2 text-xs text-[var(--text2)]">
              <input id="cv-use-ai" type="checkbox" checked={useAiMatch} onChange={(e) => setUseAiMatch(e.target.checked)} />
              <label htmlFor="cv-use-ai">{T('cv_ai_use_deepseek')}</label>
            </div>
          </div>
        ...
        {visibleProfiles.length > 0 && (
          <p className="text-sm text-[var(--text2)] mb-5">
            {visibleProfiles.length === 1
              ? T('cv_found_one')
              : T('cv_found_many').replace('{n}', String(visibleProfiles.length))}
          </p>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {visibleProfiles.map(p => (
            <CVCard
              ...
              profile={{
                ...p,
                cv_free: p.cv_free || unlockedSiteIds.has(p.id),
              }}
              onUnlock={handleUnlock}
              matchScore={jobNeed.trim() ? (useAiMatch && aiScores[p.id] != null ? aiScores[p.id] : localMatchScore(p, jobNeed)) : null}
            />
          ))}
