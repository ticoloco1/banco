...
import { Coins, ArrowRight, ArrowLeft, History, Wallet, Info, CheckCircle, ExternalLink, Shield } from 'lucide-react';
...
export default function CreditosPage() {
  ...
  const [rewardWithdrawUsd, setRewardWithdrawUsd] = useState('');
  const [sensitiveTotpCode, setSensitiveTotpCode] = useState('');
  const [totpEnabled, setTotpEnabled] = useState(false);
  const [totpHasPending, setTotpHasPending] = useState(false);
  ...
  useEffect(() => {
    ...
    (supabase as any)
      .from('user_usdc_rewards_balance')
      .select('balance_usd')
      .eq('user_id', user.id)
      .maybeSingle()
      .then(({ data }: any) => setRewardsBalanceUsd(Number(data?.balance_usd || 0)));
  }, [user]);
  ...
  const loadTotpStatus = async () => {
    ...
    const res = await fetch('/api/platform/security/totp', {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    });
    const data = await res.json().catch(() => ({}));
    if (res.ok) {
      setTotpEnabled(Boolean(data?.enabled));
      setTotpHasPending(Boolean(data?.hasPending));
    }
  };
  ...
  const handleRewardsWithdraw = async () => {
    const amount = Number(rewardWithdrawUsd || 0);
    if (!Number.isFinite(amount) || amount <= 0) {
      toast.error('Invalid cashback withdraw amount');
      return;
    }
    if (!walletAddress) {
      toast.error(T('err_wallet_polygon_first'));
      return;
    }
    const token = await authToken();
    const res = await fetch('/api/platform/rewards/withdraw', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
      },
      body: JSON.stringify({ amountUsd: amount, walletAddress, totpCode: sensitiveTotpCode || undefined }),
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      toast.error(data?.error || 'Withdraw request failed');
      return;
    }
    setRewardsBalanceUsd(Number(data?.balanceUsd || 0));
    setRewardWithdrawUsd('');
    setSensitiveTotpCode('');
    toast.success('Cashback withdraw requested.');
  };
  ...
  const handleTotpSetup = async () => {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) return toast.error(data?.error || 'TOTP setup failed');
    setTotpSecret(String(data?.secret || ''));
    setTotpOtpAuthUrl(String(data?.otpauthUrl || ''));
    setTotpHasPending(true);
    toast.success('TOTP secret generated. Confirm with a 6-digit code.');
  };
  ...
  const handleTotpEnable = async () => {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) return toast.error(data?.error || 'Invalid TOTP code');
    setTotpCode('');
    setTotpSecret('');
    setTotpOtpAuthUrl('');
  ...
  const handleTotpDisable = async () => {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) return toast.error(data?.error || 'Unable to disable TOTP');
    setTotpCode('');
    setTotpSecret('');
    setTotpOtpAuthUrl('');
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)]">
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="card p-5 mb-8 border border-emerald-500/25 bg-emerald-500/5">
          <h2 className="font-black text-[var(--text)] text-xl mb-2">Pro Cashback (USDC)</h2>
          <p className="text-sm text-[var(--text2)] mb-3">
            Internal rewards balance from Pro cashback on marketing leads.
          </p>
          <p className="text-2xl font-black text-emerald-400 mb-3">${rewardsBalanceUsd.toFixed(2)} USD</p>
          <div className="flex flex-wrap gap-2 items-center">
            <input
              ...
              value={rewardWithdrawUsd}
              onChange={(e) => setRewardWithdrawUsd(e.target.value)}
            />
            <button className="btn-primary text-sm" onClick={() => void handleRewardsWithdraw()}>
              Request USDC withdraw
            </button>
          </div>
          {totpEnabled && (
            <div className="mt-3">
              <input
                ...
                maxLength={6}
                className="input max-w-[180px]"
                placeholder="TOTP 6 dígitos"
                value={sensitiveTotpCode}
                onChange={(e) => setSensitiveTotpCode(e.target.value.replace(/\D/g, '').slice(0, 6))}
              />
              <p className="text-xs text-[var(--text2)] mt-1">
                TOTP exigido para saque e edição de wallet.
              </p>
            </div>
          )}
        </div>

        <div className="card p-5 mb-8 border border-blue-500/25 bg-blue-500/5">
          <h2 className="font-black text-[var(--text)] text-xl mb-2 flex items-center gap-2">
            <Shield className="w-5 h-5 text-blue-400" /> Segurança (Google Authenticator)
          </h2>
          <p className="text-sm text-[var(--text2)] mb-3">
            Ative TOTP opcional para proteger ações sensíveis.
          </p>
          <div className="flex flex-wrap gap-2 items-center mb-3">
            <span className="text-xs px-2 py-1 rounded-full border border-[var(--border)]">
              {totpEnabled ? 'TOTP ativo' : 'TOTP inativo'}
            </span>
            {!totpEnabled && (
              <button className="btn-secondary text-sm" onClick={() => void handleTotpSetup()}>
                Gerar segredo TOTP
              </button>
            )}
          </div>
          {(totpHasPending || totpSecret) && (
            <div className="rounded-xl border border-[var(--border)] p-3 bg-[var(--bg2)] mb-3">
              {totpOtpAuthUrl ? (
                <img
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(totpOtpAuthUrl)}`}
                  alt="QR Code TOTP"
                  className="w-[180px] h-[180px] rounded-md mb-2"
                />
              ) : null}
              {totpSecret ? <p className="text-xs font-mono break-all">{totpSecret}</p> : null}
              <p className="text-xs text-[var(--text2)] mt-2">Escaneie no Google Authenticator e confirme com o código de 6 dígitos.</p>
            </div>
          )}
          <div className="flex flex-wrap items-center gap-2">
            ...
            />
            {!totpEnabled ? (
              <button className="btn-primary text-sm" onClick={() => void handleTotpEnable()}>
                Ativar TOTP
              </button>
            ) : (
              <button className="btn-secondary text-sm" onClick={() => void handleTotpDisable()}>
                Desativar TOTP
              </button>
            )}
          </div>
