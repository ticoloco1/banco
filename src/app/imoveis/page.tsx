...
import { OsmEmbedMap } from '@/components/classified/OsmEmbedMap';
...
function imovelHighlightActive(until: string | null | undefined) {
  if (!until) return false;
  return new Date(until).getTime() > Date.now();
}

function hex6FromHighlight(raw: string | null | undefined, fb: string) {
  const s = String(raw || '')
    .replace('#', '')
    .replace(/[^0-9a-f]/gi, '')
    .toLowerCase()
    .slice(0, 6);
  return s.length === 6 ? s : fb;
}

function sortImovelListings(rows: Listing[]): Listing[] {
  return rows.slice().sort((a, b) => {
    const aH = imovelHighlightActive(a.highlight_until) ? 1 : 0;
    const bH = imovelHighlightActive(b.highlight_until) ? 1 : 0;
    if (aH !== bH) return bH - aH;
    const bs = Number(b.boost_score ?? 0) - Number(a.boost_score ?? 0);
    if (bs !== 0) return bs;
    return new Date(b.created_at || 0).getTime() - new Date(a.created_at || 0).getTime();
  });
}

/** Dono do anúncio: cor + carrinho para destaque pago no diretório (30d, USD). */
function ImovelHighlightControls({ item, T, layout }: { item: Listing; T: (k: MessageKey) => string; layout: 'card' | 'modal' }) {
  const { user } = useAuth();
  const addToCart = useCart((s) => s.add);
  const [color, setColor] = useState(() => hex6FromHighlight(item.highlight_color, 'f59e0b'));
  useEffect(() => {
    setColor(hex6FromHighlight(item.highlight_color, 'f59e0b'));
  }, [item.id, item.highlight_color]);

  const isOwner = Boolean(user?.id && item.user_id && item.user_id === user.id);
  if (!isOwner) return null;

  const active = imovelHighlightActive(item.highlight_until);
  const price = PLATFORM_USD.propertyDirectoryHighlight30d;
  const priceStr = price.toFixed(2);

  const onAdd = () => {
    const colorHex = hex6FromHighlight(color, 'f59e0b');
    addToCart({
      id: `classified_highlight_${item.id}__${colorHex}`,
      label: T('imoveis_cart_highlight_label').replace('{price}', priceStr),
      price,
      type: 'classified_highlight',
    });
    toast.success(T('imoveis_toast_highlight_cart'));
  };

  const wrap =
    layout === 'card'
      ? 'flex flex-wrap items-center gap-2 pt-2 mt-2 border-t border-slate-100 dark:border-slate-800'
      : 'flex flex-wrap items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700';

  return (
    <div className={wrap}>
      {layout === 'card' && (
        <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 w-full basis-full uppercase tracking-wide">
          {T('imoveis_highlight_card_eyebrow')}
        </span>
      )}
      {layout === 'modal' && (
        <p className="text-xs text-slate-500 w-full basis-full">{T('imoveis_highlight_hint')}</p>
      )}
      <input
        type="color"
        value={`#${hex6FromHighlight(color, 'f59e0b')}`}
        onChange={(e) => setColor(hex6FromHighlight(e.target.value, 'f59e0b'))}
        onClick={(e) => e.stopPropagation()}
        className="w-8 h-8 p-0 border border-slate-200 dark:border-slate-600 rounded-lg cursor-pointer bg-transparent flex-shrink-0"
        title={T('imoveis_highlight_color_picker_title')}
      />
      <button
        type="button"
        onClick={(e) => {
        ...
        className="text-xs px-3 py-2 rounded-xl font-bold border border-amber-500/50 text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/40 hover:bg-amber-100 dark:hover:bg-amber-950/70 transition-colors"
      >
        {active
          ? T('imoveis_highlight_renew').replace('{price}', priceStr)
          : T('imoveis_highlight_buy').replace('{price}', priceStr)}
      </button>
    </div>
  );
}
  ...
  return (
    <div className="relative aspect-[4/3] overflow-hidden group bg-black">
      <div className="absolute inset-0 transition-all duration-500 flex" style={{ transform:`translateX(-${idx*100}%)`, width:`${images.length*100}%` }}>
        {images.map((img,i) => <img key={i} src={getAssetUrl(img)} alt="" className="w-full h-full object-cover flex-shrink-0" style={{ width:`${100/images.length}%` }} />)}
      </div>
...
function Lightbox({ images, startIdx, onClose }: { images:string[]; startIdx:number; onClose:()=>void }) {
  ...
  return (
    <div className="fixed inset-0 z-[999] bg-black/96 flex items-center justify-center" onClick={onClose}>
      ...
      <img src={getAssetUrl(images[idx])} alt="" className="max-h-[88vh] max-w-[88vw] object-contain rounded-xl" onClick={e=>e.stopPropagation()}/>
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 overflow-x-auto px-4" onClick={e=>e.stopPropagation()}>
        {images.map((img,i) => <button key={i} onClick={()=>setIdx(i)} className={`flex-shrink-0 w-14 h-10 rounded overflow-hidden border-2 transition-all ${i===idx?'border-white':'border-transparent opacity-40 hover:opacity-70'}`}><img src={getAssetUrl(img)} className="w-full h-full object-cover"/></button>)}
      </div>
    </div>
  );
}
...
function DetailModal({ item, onClose }: { item:Listing; onClose:()=>void }) {
  ...
  const modalHl = imovelHighlightActive(item.highlight_until);
  const modalHlHex = item.highlight_color?.trim()
    ? item.highlight_color.startsWith('#')
      ...
      <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onClose}>
        <div className="bg-white dark:bg-slate-900 w-full max-w-3xl max-h-[95vh] overflow-y-auto rounded-t-3xl sm:rounded-2xl shadow-2xl" onClick={e=>e.stopPropagation()}>
          <div className="relative">
            ...
            : images.length===1 ? <img src={getAssetUrl(images[0])} className="w-full aspect-[16/9] object-cover cursor-zoom-in" onClick={()=>setLbIdx(0)}/>
            : <div className="grid grid-cols-4 grid-rows-2 gap-1 h-72">
                <div className="col-span-2 row-span-2 overflow-hidden cursor-zoom-in" onClick={()=>setLbIdx(0)}><img src={getAssetUrl(images[0])} className="w-full h-full object-cover hover:brightness-90 transition-all"/></div>
                {images.slice(1,5).map((img,i) => <div key={i} className="overflow-hidden relative cursor-zoom-in" onClick={()=>setLbIdx(i+1)}><img src={getAssetUrl(img)} className="w-full h-full object-cover hover:brightness-90 transition-all"/>{i===3&&images.length>5&&<div className="absolute inset-0 bg-black/60 flex items-center justify-center"><span className="text-white font-black text-xl">+{images.length-5}</span></div>}</div>)}
              </div>}
          ...
          <div className="p-6">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {modalHl && (
                    <span
                      ...
                      style={{ backgroundColor: `${modalHlHex}22`, color: modalHlHex }}
                    >
                      {T('imoveis_highlight_badge')}
                    </span>
                  )}
                </div>
            ...
            <div className="mb-4">
              <ImovelHighlightControls item={item} T={T} layout="modal" />
            </div>
            {typeof item.extra?.lat === 'number' && typeof item.extra?.lng === 'number' && (
              <div className="mb-6">
                <h3 className="font-bold text-slate-800 dark:text-slate-200 mb-2 flex items-center gap-2">
                  <MapPin className="w-4 h-4 text-blue-500" /> {T('classified_detail_map')}
                </h3>
                <OsmEmbedMap lat={item.extra.lat} lng={item.extra.lng} />
                {item.extra.map_label && (
                  <p className="text-xs text-slate-500 mt-2">{item.extra.map_label}</p>
                )}
              </div>
            )}
            {/* Boost */}
            ...
            {item.mini_sites && (
              <div className="border border-slate-200 dark:border-slate-700 rounded-2xl p-4 flex items-center gap-4">
                {item.mini_sites.avatar_url ? <img src={getAssetUrl(item.mini_sites.avatar_url)} className="w-12 h-12 rounded-full object-cover"/> : <div className="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center"><Home className="w-5 h-5 text-blue-500"/></div>}
...
export default function ImoveisPage() {
  ...
  const load = useCallback(async (reset=false) => {
    ...
    const { data } = await q;
    const raw = data || [];
    const batchSorted = sortImovelListings(raw);
    setItems((prev) => sortImovelListings(reset ? batchSorted : [...prev, ...batchSorted]));
    setHasMore(batchSorted.length === PAGE_SIZE);
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {items.map(item => {
            const hlOn = imovelHighlightActive(item.highlight_until);
            const hlHex = item.highlight_color?.trim()
              ? item.highlight_color.startsWith('#')
                ? item.highlight_color
                : `#${item.highlight_color}`
              : '#f59e0b';
            ...
            <article
              key={item.id}
              className={`group bg-white dark:bg-slate-900 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer hover:-translate-y-1 ${
                hlOn ? 'border-2' : 'border border-slate-100 dark:border-slate-800'
              }`}
              style={
                hlOn
                  ? {
                      borderColor: `${hlHex}99`,
                      boxShadow: `0 0 0 1px ${hlHex}40 inset, 0 8px 24px -8px ${hlHex}33`,
                    }
                  : undefined
              }
            ...
            >
              ...
              <div className="p-4">
                <div className="flex items-start justify-between mb-1 gap-2">
                  ...
                  {hlOn && (
                    <span
                      ...
                      style={{ backgroundColor: `${hlHex}22`, color: hlHex }}
                    >
                      {T('imoveis_highlight_badge')}
                    </span>
                  )}
                ...
                <div className="flex items-center gap-3 text-xs text-slate-600 dark:text-slate-400 border-t border-slate-100 dark:border-slate-800 pt-3">
                  ...
                  {item.mini_sites && (
                    <span className="ml-auto flex items-center gap-1">
                      {item.mini_sites.avatar_url && <img src={getAssetUrl(item.mini_sites.avatar_url)} className="w-5 h-5 rounded-full object-cover"/>}
                ...
                <ImovelHighlightControls item={item} T={T} layout="card" />
              </div>
            </article>
            );
          })}
