...
import { isValidYouTubeVideoId } from '@/lib/utils';
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 flex-1">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {videos.map(video => {
            ...
            const hasYtThumb = isValidYouTubeVideoId(video.youtube_video_id || '');
            const thumb = hasYtThumb ? `https://img.youtube.com/vi/${video.youtube_video_id}/maxresdefault.jpg` : '';
            return (
                ...
                <div className="relative aspect-video bg-black">
                  {playing === video.id ? (
                    <SecureVideoPlayer videoId={video.id} title={video.title}
                      ...
                      previewImageUrl={video.preview_image_url}
                      previewEmbedUrl={video.preview_embed_url}
                    />
                  ) : (
                    <div className="relative w-full h-full cursor-pointer" onClick={() => setPlaying(video.id)}>
                      {!video.paywall_enabled && hasYtThumb
                  ...
                  <div className="flex items-center justify-between">
                    <a href={`https://${video.mini_sites?.slug}.trustbank.xyz`} target="_blank" rel="noopener"
                      ...
                      {video.mini_sites?.avatar_url
                        ? <img src={getAssetUrl(video.mini_sites.avatar_url)} className="w-6 h-6 rounded-full object-cover" />
