...
export async function validateCartCreatorsHaveStripe(
  ...
  for (const item of items) {
    ...
    if (line.kind === 'classified_property_highlight' && line.itemId && payerUserId) {
      const expected = PLATFORM_USD.propertyDirectoryHighlight30d;
      if (!priceClose(Number(item.price), expected)) {
        return {
          ok: false,
          error: 'Property highlight price changed — refresh the page and try again.',
        };
      }
      const listingId = String(line.itemId).split('::')[0];
      if (!/^[0-9a-f-]{36}$/i.test(listingId)) {
        return { ok: false, error: 'Invalid property highlight item.' };
      }
      const { data: cl } = await db
        ...
        .eq('id', listingId)
        .maybeSingle();
      if (!cl || (cl as { user_id: string }).user_id !== payerUserId) {
        return { ok: false, error: 'You can only buy a highlight for your own property listing.' };
      }
      if ((cl as { type: string }).type !== 'imovel') {
        return { ok: false, error: 'Highlights apply to property listings only.' };
      }
    }

    if (line.kind === 'profile_loop_video' && line.itemId && payerUserId) {
      const annual =
        line.billingPeriod === 'yearly' || line.billingPeriod === 'annual';
      ...
      if (!priceClose(Number(item.price), expected)) {
        return {
          ok: false,
          error: 'O preço do add-on de vídeo em loop mudou — atualiza a página e tenta de novo.',
        };
      }
      const { data: site } = await db.from('mini_sites').select('user_id').eq('id', line.itemId).maybeSingle();
      if (!site || (site as { user_id: string }).user_id !== payerUserId) {
        return {
          ok: false,
          error: 'Só podes comprar gravações longas para o teu próprio mini-site.',
        };
      }
    }

    if (line.kind === 'feed_subscription' && line.itemId && payerUserId) {
      const days = line.feedPassDays === 365 ? 365 : 7;
      const expected =
        days === 365 ? PLATFORM_USD.feedDirectory365d : PLATFORM_USD.feedDirectory7d;
      if (!priceClose(Number(item.price), expected)) {
        return { ok: false, error: 'Feed pass price changed — refresh and try again.' };
      }
      const { data: site } = await db.from('mini_sites').select('user_id').eq('id', line.itemId).maybeSingle();
      if (!site || (site as { user_id: string }).user_id !== payerUserId) {
        return { ok: false, error: 'You can only buy a feed pass for your own mini-site.' };
      }
    }
    ...
    if (line.kind === 'marketing_campaign_setup' && line.itemId && payerUserId) {
      const budget = Number(line.faceValueUsd ?? 0);
      if (!Number.isFinite(budget) || budget < 0) {
        return { ok: false, error: 'Orçamento inválido para campanha de marketing.' };
      }
      const expected = marketingSetupFeeUsd() + budget;
      if (!priceClose(Number(item.price), expected)) {
        return { ok: false, error: 'Valor da campanha mudou — atualiza e tenta novamente.' };
      }
      const { data: campaign } = await db
        ...
        .eq('id', line.itemId)
        .maybeSingle();
      if (!campaign || (campaign as { brand_id?: string }).brand_id !== payerUserId) {
        return { ok: false, error: 'Só a marca dona pode pagar esta campanha.' };
      }
    }
  }
