...
export async function fulfillCheckoutPendingRow(
  ...
  let polygonWallet = typeof pwRaw === 'string' && pwRaw.trim() ? pwRaw.trim() : null;
  if (polygonWallet && isValidEvmAddress(polygonWallet)) polygonWallet = polygonWallet.toLowerCase();
  else if (polygonWallet && !isValidEvmAddress(polygonWallet)) polygonWallet = null;

  const needsSlugWallet = (lines || []).some((l: { kind?: string }) =>
    l.kind === 'slug' || l.kind === 'slug_market' || l.kind === 'slug_auction_settle',
  );
  if (needsSlugWallet && (!polygonWallet || !isValidEvmAddress(polygonWallet))) {
    const fallback = await getUserPolygonWallet(db, userId);
    if (fallback) polygonWallet = fallback;
  }
